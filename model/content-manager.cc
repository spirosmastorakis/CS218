...
uint32_t
ContentManager::GetCommunityIdOfOwnerOfContent(Ipv4Address contentOwner) const
{
    for (uint32_t i=0; i<m_size; ++i)
    {
        if (m_array[i].contentOwner.IsEqual(contentOwner))
        {
            return m_array[i].communityId;
        }
    }
    return 0;
}
