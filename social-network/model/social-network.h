...
struct PendingResponseEntry //each entry is uniquely identified by <requesterId, broadcastId>
{
    ...
    {
        this->lastRelayNode = Ipv4Address("0.0.0.0");
    }
                         ...
                         Ipv4Address contentRequested, uint32_t broadcastId)
    {
        ...
        this->lastRelayNode = Ipv4Address("0.0.0.0"); //similar to NULL address
    ...
    Ipv4Address contentDestinationNode; //used when a node knows which node has a conent
...
private:
    ...
    PktHeader *CreateDataPacketHeader(Ipv4Address destinationId, Ipv4Address requesterId, Ipv4Address contentRequested);
