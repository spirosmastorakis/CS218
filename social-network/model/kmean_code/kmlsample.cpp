...
void GenerateCenters(int k, CentralityTableEntry * myList, int ValueLength, double * centers, int commID)
{
	...
	{
		int community = myList[i].node.commID;
		if(commID == community)
		{	
			match++;
		}
    ...
    KMdata dataPts(1, match);		// allocate data storage
	...
	for (int i = 0; i < ValueLength; i++) 		
	{
		...
		if(community == commID)
		{
			pts[indnew][0] = centrality;	
			indnew++;
		}
	}
    
	dataPts.setNPts(match);			// set actual number of pts
	...
	*/
	...
	for(int i = 0; i < ValueLength; i++)
	{
		...
		if(commID == community)
		{
			group = AssignGroup(centr,centers,k);
